...
#define NORMAL_MODE_HI_VOLTAGE		74   // ~0.2V
#define DEBUG_MODE_LOW_VOLTAGE		256  // ~0.8V
#define DEBUG_MODE_HI_VOLTAGE			377  // ~1.2V
...
#elif defined(__AVR_ATmega164A__) || defined(__AVR_ATmega164PA__)
//#define RAMSTART (0x100)
#define NRWWSTART (0x1C00)
#elif defined(__AVR_ATmega324A__) || defined(__AVR_ATmega324PA__)
//#define RAMSTART (0x100)
#define NRWWSTART (0x3800)
#elif defined(__AVR_ATmega644A__) || defined(__AVR_ATmega644PA__)
//#define RAMSTART (0x100)
#define NRWWSTART (0x7000)
#elif defined(__AVR_ATmega1284A__) || defined(__AVR_ATmega1284P__)
//#define RAMSTART (0x100)
...
int main(void)
{
    ...
    //delay to wait for things to stablize
    //TCNT1 = -(F_CPU/(1024*8));
    //TIFR1 = _BV(TOV1);
    ...
    //read eeprom flash flag
    doFlash = eeprom_read_byte(EEPROM_PROGRAM_FLASH_ON_BOOT_ADDRESS);


    ADMUX = 0x47; //AREF and input 7
    DIDR0 = 0xFF; //disable digital input buffers on all ADC lines
    ADCSRA = 0xC7; //enable, start conversion, and clock div factor 64
    ADC_WAIT_FOR_CONVERSION;

    // PINB1 being set will get us in to Bed of Nails Mode
    if(bit_is_set(PINB, PINB1))
    {
        appStart();
    }

    // 1V on DEVICEDET or doFlash being equal to 0xAA will give you
    // 4 seconds to either start programming or enter debug mode
    if(!((ADC <= DEBUG_MODE_HI_VOLTAGE) &&
    (ADC > DEBUG_MODE_LOW_VOLTAGE)) &&
    !(doFlash == 0xAA))
    {
        appStart();
    }
    ...
    // Set up watchdog to trigger after 1 second
